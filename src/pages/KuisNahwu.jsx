...
const KuisNahwu = () => {
  ...
  const fetchQuizData = useCallback(() => {
    axios
      .get(`https://enormous-mint-tomcat.ngrok-free.app/admin/quiz/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "ngrok-skip-browser-warning": "true",
        },
  ...
  const handleSaveChanges = (index) => {
    if (isAdding) {
      ...
      axios
        .post(
          `https://enormous-mint-tomcat.ngrok-free.app/admin/quiz/soal/${id}`,
          {
        ...
        <div className="bg-white rounded-l-3xl min-h-[98.75vh] h-full pt-5 pl-4 pr-4">
          <div className="border rounded-xl flex flex-col p-2 mb-4 ml-2">
            <div className="flex items-center">
              <NavLink to={`/nahwutopik/${id}`}>
                <ArrowBackIcon className="text-dark-cyan" />
              </NavLink>
              <p className="text-dark-cyan text-2xl font-semibold ml-2">Kuis</p>
            </div>
            <p className="text-sm">Anda dapat melakukan pengelolaan kuis</p>
            <div className="flex items-center">
              <a
                href={`/nahwu/${kategoriMateri}`}
              ...
              />
              <a href={`/nahwutopik/${id}`} className="text-[#6D6D6D] text-sm">
                Topik
              </a>
              <PlayArrowIcon
                style={{ fontSize: "16px" }}
          ...
          <div>
            ...
            {quizData.length > 0 ? (
              ...
              <p className="text-center mt-4">
                Tidak ada data kuis yang tersedia.
              </p>
            )}
          </div>
          <div>
